...
// Modified by Lyle McGeoch, 2/2008 to include dumping of itemsets, etc.
...
public final class Grammar
{
  ...
  static java.io.PrintWriter dumpout;
  ...
  public static void reinit(boolean amhDumping)
  {
    fastLr0Closure = new TreeMap();
    fastLr1Closure = new TreeMap();
    startSymbol = 0;
    ...
    try {
	if (amhDumping) dumpout = new java.io.PrintWriter (new java.io.FileWriter("parser.dump"));
    }
  ...
  public static void computeLALR() throws ConflictException
  {
    ...
    if (dumpout != null) {
    	for (int i=0; i<FIRST_Nonterminal.length; ++i)
    	    dumpout.println ("FIRST of " + Symbol.nonterminals()[i] + " is " + FIRST_Nonterminal[i]);
	//    	for (int i=0; i<FOLLOW.length; ++i)
	//    	    dumpout.println ("FOLLOW of " + Symbol.nonterminals()[i] + " is " + FOLLOW[i]);
    }
	...
	for (int i = 0; i<sets.length; ++i) {
	    dumpout.println ("Item set " + i + "---");
	    LR0Item[] items = sets[i].items();
	    for (int j=0; j<items.length; ++j)
		dumpout.println (items[j] + ": " + collection.lookaheads[i].get(items[j]));
	}
	dumpout.println();
	dumpout.flush();
    }
